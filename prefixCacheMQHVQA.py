...
def call_vllm_api_with_embeds(image_embedding, question="What's in this image?", model="llava-hf/llava-1.5-7b-hf", api_url="http://localhost:8005", guided_choice=None):
    ...
    if guided_choice and isinstance(guided_choice, list) and len(guided_choice) > 0:
        ...
        print(f"  🎯 Using default guided_choice: ['yes', 'no']")
...
#     try:
#         with torch.no_grad():
#             # Ensure model components are on correct device and dtype
#             model.language_model = model.language_model.to(device=model_device, dtype=torch.float16)
#             model.tokenizer.model_max_length = max_pos
            
#             # Tokenize texts individually
#             text_inputs = [model.tokenizer(text, return_tensors="pt", truncation=True, max_length=max_pos) for text in texts]
            
#             # Stack inputs for batch processing
#             input_ids = torch.nn.utils.rnn.pad_sequence(
#                 [ti['input_ids'].squeeze(0) for ti in text_inputs],
#                 batch_first=True,
#                 padding_value=model.tokenizer.pad_token_id
#             ).to(model_device, dtype=torch.long)
#             print(f"Input IDs dtype: {input_ids.dtype}, device: {input_ids.device}")
            
#             # Process through language model
#             text_embeds = model.language_model(input_ids=input_ids).last_hidden_state  # [num_texts, seq_len, embed_dim]
#             text_embeds = text_embeds.to(dtype=torch.float16, device=model_device)
#             text_embeds = text_embeds.mean(dim=1)  # [num_texts, embed_dim]
#             print(f"Text embeds dtype: {text_embeds.dtype}, device: {text_embeds.device}")
#             return text_embeds
...
#     if num_diverse_tokens > 0 and len(remaining_indices) > 0 and texts:
#         # Encode individual questions
#         text_embeds = process_text_efficiently(texts, model, model_device)
#         if text_embeds is None:
#             print("Falling back to visual attention only due to text processing failure")
#             final_scores = cls_attention[remaining_indices]
#         else:
#             text_embeds = text_embeds.unsqueeze(0).to(dtype=torch.float16, device=model_device)  # [1, num_questions, embed_dim]
#             print(f"Text embeds shape: {text_embeds.shape}, dtype: {text_embeds.dtype}, device: {text_embeds.device}")
...
#             fused_guidance = mqa(text_embeds)  # [1, 1, embed_dim]
#             fused_guidance = fused_guidance.to(dtype=torch.float16, device=model_device)
#             print(f"Fused guidance dtype: {fused_guidance.dtype}, device: {fused_guidance.device}")
...
#                 projection = nn.Linear(fused_guidance.shape[-1], remaining_features.shape[-1], bias=False).to(device=model_device, dtype=torch.float16)
#                 fused_guidance = projection(fused_guidance)
#                 print(f"Projection output dtype: {fused_guidance.dtype}, device: {fused_guidance.device}")
            
#             text_scores = torch.einsum('btd,bmd->bm', fused_guidance, remaining_features)
#             text_scores = F.softmax(text_scores, dim=-1)  # [B, M]
...
#     end = time.time()
#     print(f"Step 2 (MQAF): {end - begin:.2f}s")
    
#     # Step 3: Recovery with text guidance
#     if texts and recovery_ratio > 0:
#         selected_mask = torch.zeros(N, dtype=torch.bool, device=model_device)
#         selected_mask[selected_indices] = True
#         pruned_indices = torch.nonzero(~selected_mask, as_tuple=True)[0]
...
#             num_tokens_to_recover = min(int(recovery_ratio * N), len(pruned_indices))
#             if num_tokens_to_recover > 0:
#                 if 'fused_guidance' not in locals():
#                     text_embeds = process_text_efficiently(texts, model, model_device)
#                     if text_embeds is not None:
#                         text_embeds = text_embeds.unsqueeze(0).to(dtype=torch.float16, device=model_device)
#                         mqa = MultiQueryAttention(embed_dim=text_embeds.shape[-1], device=model_device, dtype=torch.float16)
#                         fused_guidance = mqa(text_embeds)
...
if __name__ == "__main__":
    ...
    try:
        ...
        image_counts = df.groupby('image_id').size().sort_values(ascending=False)
            ...
            for image_idx, (image_source, image_group) in enumerate(image_groups):
                ...
                for q_idx, (_, row) in enumerate(questions_for_image.iterrows()):
                    ...
                    result_record = {
                        ...
                        'guided_choices_used': str(guided_choices) if guided_choices else "['yes', 'no']",  # NEW: Track guided choices
