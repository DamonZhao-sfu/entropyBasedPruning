...
def getPrunedVisualToken(model, image_path, texts, keep_ratio=0.125, lambda_val=0.1):
    ...
    if texts is not None:
        ...
        # Fallback to vision tower's attention if no text is provided
        attention_matrix = image_forward_outs.attentions[-1]  # [B, num_heads, seq_len, seq_len]
        num_image_tokens = N
        num_text_tokens = text_inputs['input_ids'].shape[1] if texts else 0
        attention_matrix = attention_matrix[:, :, :num_text_tokens, 1:num_image_tokens+1]  # [B, num_heads, num_text_tokens, num_image_tokens]
        attention_matrix = attention_matrix.mean(dim=1)  # [B, num_text_tokens, num_image_tokens]
        attention_matrix = attention_matrix[0]  # [num_text_tokens, num_image_tokens]
        attention_matrix = F.softmax(attention_matrix, dim=1)  # Normalize over image tokens
